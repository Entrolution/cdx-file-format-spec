...
Each asset category declared in the manifest MUST have its own index file located at `assets/<category>/index.json`, where `<category>` is the corresponding key in the manifest's `assets` object. The manifest's `assets.<category>.index` path MUST equal this derived location. A reader MUST verify that a declared `index` equals this derived location, comparing after path normalization (no `.` or `..` segments; case-sensitive), and MUST reject the document otherwise (State Machine section 5.4.2); it then loads the index from that path. Document Hashing section 4.3.1 builds every asset archive path from the category key, never from the `index` field, so the two cannot both be authoritative: a divergent `index` would leave the manifest projection binding the hash of one file (Security Extension section 9.7) while the document ID inherits the asset hashes listed in the file at the derived path, so a signature would attest a file that identity does not depend on, and two readers could resolve the same category differently. Rejecting the divergence removes the ambiguity — for every loadable document the declared and derived paths name one file. Asset `path` values within an index are resolved relative to that category directory. The index catalogs all assets in the category:
