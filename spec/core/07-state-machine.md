...
- **A stored-byte Unicode violation always REJECTs.** A hashed object key or string value that is not well-formed Unicode (an unpaired surrogate code point), or that is not in Normalization Form C — individually or across the concatenated text content of a block — MUST be rejected in *every* state (Document Hashing section 4.3.2). Section 4.3.2 forbids normalizing at hash time, so neither defect can be repaired without changing the document's identity: an ill-formed string leaves two conformant readers computing different document IDs from one archive, and a non-NFC block leaves every character offset into it (Anchors and References section 3) addressing text the document does not contain. Neither is a producer slip a consumer can absorb.
