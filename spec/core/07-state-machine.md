...
**Integrity-indeterminate is not a disposition.** The four values above are responses to a *detected failure*. A reader may also be unable to establish whether a failure exists at all — because material is encrypted and it holds no key (note 5), or compressed with a permitted method it has not implemented (note 6). That outcome is reported as **integrity-indeterminate**: it asserts neither success nor failure, and it is not one of the four dispositions and does not compose with them. A reader MUST NOT collapse it to IGNORE, which would present unverified material as checked.
...
| An archive entry declaring a compression method outside the set Container Format section 3.2 permits (Store, Deflate, Zstandard) — the entry's bytes cannot be obtained, so neither its CRC-32 nor any hash computed over it can be verified (see note 6 for a *permitted* method the reader does not implement) | REJECT | REJECT |
| An archive entry whose data cannot be decoded under its declared, *permitted* compression method — corrupt or truncated bytes, so no CRC-32 can be computed for it at all (Container Format sections 3.2, 6.1). A container-layer physical-corruption check, applying regardless of the entry's logical layer, exactly as the CRC-32 row above | REJECT | REJECT |
...
| A hashed object key or string value that is not well-formed Unicode — it carries an unpaired surrogate code point (Document Hashing section 4.3.2), and so has no UTF-8 encoding at all. RFC 8259 parsers disagree on such an escape exactly as they disagree on a duplicate key: one preserves the lone surrogate, another substitutes U+FFFD, a third refuses the input — so two conformant readers derive different canonical bytes, and therefore different document IDs, from one archive, and each can find the other's signature invalid. This is a split-view substitution, not a producer slip | REJECT | REJECT |
| A hashed object key or string value that is not in Normalization Form C, whether individually or in the concatenated text content of a run of adjacent text nodes within a block-content array — a combining sequence split across text-node boundaries is the same defect (Document Hashing section 4.3.2). Section 4.3.2 makes NFC a property of the stored bytes and forbids normalizing at hash time, so a reader has no repair available: normalizing would change the very bytes the document is identified by. Character offsets are defined over NFC text (Anchors and References section 3), so a non-NFC block leaves every anchor, range, and annotation addressing it pointing into text the document does not contain — a split view of the addressing layer, distinct from the parse-level split view above | REJECT | REJECT |
| Manifest absent, unparseable, or missing or mistyping a required field | REJECT | REJECT |
| A declared `manifest.assets.<category>.index` that does not equal the derived location `assets/<category>/index.json`, compared after the path normalization of Document Hashing section 4.3.1 (no `.` or `..` segments; case-sensitive). Asset Embedding section 3.1 states the equality as a MUST, and a divergence leaves the document ambiguous: a reader honouring the declaration and one honouring the derivation would load different bytes as the index, and so resolve the category's assets differently | REJECT | REJECT |
...
| Unrecognized file or directory in the archive (Container Format) — except an undeclared presentation or precise-layout file, which the dedicated row below governs | IGNORE | IGNORE |
...
| The `content` part is present and parses, but violates the content **root envelope** — the parsed value is not a JSON object, or `version` or `blocks` is absent or of the wrong JSON type (Content Blocks section 2.2 makes both Required, section 8.1 makes validating the root a load step, and section 7.1 item 5 states the rejection as a MUST). The part's absence and its unparseability are disposed of elsewhere (the row above; section 5.4.1); this row covers the case between them. A mistyped `blocks` is not cosmetic: every content-level row in this table is reached by walking `blocks`, so a `blocks` that is not an array silently withdraws all of them — the unknown-bare-type REJECT above included — turning a document this table rejects into one nothing reports. Nor is absent the same as empty: an absent `blocks` and `blocks: []` canonicalize to different bytes (Document Hashing section 4.3.1 item 6 preserves absence and never materializes a default), so reading the former as an empty document would give one rendered document two identities | REJECT | REJECT |
| Missing required metadata at the PART level — the Dublin Core part is referenced but absent or unparseable, or a required term is missing. An absent or mistyped `metadata` or `metadata.dublinCore` *field* is not this row: the manifest is then missing a required field, which the manifest row above rejects | WARNING | INTEGRITY-ERROR |
| Missing or unparseable referenced part bound by the document hash or manifest projection, other than the `content` part, whose absence the row above rejects and whose unparseability section 5.4.1 rejects — e.g. a declared `presentation` layer, or a `{path, hash}` side file an extension config slot declares. A part whose bytes the reader could not obtain at all is not *unparseable* for this row: see notes 5 and 6 | WARNING | INTEGRITY-ERROR |
...
| Dangling presentation reference — a `blockId`, `blockRefs`, or `blockIds` targeting a non-existent content block (Presentation Layers section 13.4) | WARNING | WARNING |
| A presentation or precise-layout file present in the archive but not declared in `manifest.presentation[]`, so no hash binds it and no manifest-covering signature attests it (Presentation Layers sections 3.4 and 12). Membership is decided by the file's own discriminator, not by its location: a precise layout carries `presentationType`, a reactive presentation a `type` of `paginated`, `continuous`, or `responsive` (section 13.3). Any other file under `presentation/` is unrecognized and takes the IGNORE row above. This row, not the out-of-hash WARNING, governs such a file: an injected layout is an attempt to alter signed content, not a benign out-of-hash annotation | WARNING | INTEGRITY-ERROR |
...
| Missing or unparseable **out-of-hash extension data** part — a collaboration, phantom, or form data file, or a path-only side file the manifest references without a hash, such as `metadata.jsonld`. A semantic bibliography or glossary and an academic numbering configuration are *not* in this class: each is declared as a `{path, hash}` pair that the manifest projection binds, so the hash-bound row above governs them | WARNING | WARNING |
...
*Note 5 (encrypted parts are present, not missing)*: a referenced part whose logical path is mapped by the security extension's encryption metadata to a stored `.enc` entry satisfies the part's existence requirement — the missing-or-unparseable-part rows above MUST NOT fire for it (Security Extension section 4.7). Its hash verification is deferred to post-decryption; a consumer without the decryption key reports the document integrity-indeterminate rather than missing-part-rejected. An encrypted document declares `cdx.security` as `required: true` (Security Extension section 4.7), so a consumer without security-extension support rejects it through the ordinary unsupported-required-extension row rather than misreading the archive.

*Note 6 (an unimplemented but permitted compression method is not a defect)*: Container Format section 3.2 makes Deflate mandatory to implement and Zstandard RECOMMENDED, so a reader that cannot decompress a Zstandard entry is still conformant and the ARCHIVE is not defective. Such an entry is reported **integrity-indeterminate** — its CRC-32 and any hash over its bytes are neither verified nor failed. This is a carve-out from section 6.1's otherwise unconditional requirement to verify every entry's CRC-32: that requirement presumes the bytes can be obtained, and a reader MUST NOT manufacture a verdict from bytes it never decoded. A reader MUST NOT report such an entry as verified, and MUST NOT silently treat it as clean: an unobtainable entry whose integrity was never established is the one case where reporting nothing would misrepresent what was checked. Two consequences follow, both mirroring note 5, which governs the identical situation for an encrypted part. First, **the missing-or-unparseable-part rows above MUST NOT fire for such an entry**: it is present and intact, and reporting it missing would blame the document for the reader's capability envelope — on a frozen or published document that would escalate to an INTEGRITY-ERROR against an archive that is not damaged. Second, where the entry carries a REQUIRED part or one bound by the document hash or manifest projection, the reader cannot establish the document's identity or integrity at all, and MUST report the **document** integrity-indeterminate rather than valid — exactly as note 5 requires of a consumer without a decryption key. Only where the entry carries no required and no hash-bound content is the document genuinely unaffected, which is the case the timestamp row's precedent covers (section 5.4.3: an unverifiable proof never rejects the document, because the disposition attaches to the proof rather than the document). This note applies only to a method the specification permits; a method outside section 3.2's set is a REJECT row above, because the container itself is non-conformant.
...
- **A stored-byte Unicode violation always REJECTs.** A hashed object key or string value that is not well-formed Unicode (an unpaired surrogate code point), or that is not in Normalization Form C — individually or across the concatenated text content of a run of adjacent text nodes within a block-content array — MUST be rejected in *every* state (Document Hashing section 4.3.2). Section 4.3.2 forbids normalizing at hash time, so neither defect can be repaired without changing the document's identity: an ill-formed string leaves two conformant readers computing different document IDs from one archive, and a non-NFC block leaves every character offset into it (Anchors and References section 3) addressing text the document does not contain. Neither is a producer slip a consumer can absorb.
