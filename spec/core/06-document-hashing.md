...
| Block & anchor id labels | Canonicalized | Relabeled to position-based names (`b0`, `b1`, …); the author's chosen label does not change the ID, and references to it are rewritten to match (section 4.3.1). **Two exceptions**: a `semantic:term` and a `semantic:footnote` block id are left as authored, so the author's label there *does* change the ID and references to it are left unchanged (section 4.3.1 item 5) |
...
**Content transforms.** Apply to `content/document.json`, across all block types including `namespace:type` extension blocks — uniformly, but for the position scoping item 4 names and the id-relabeling exceptions item 5 names:
...
4. **Merge text nodes**: merge adjacent sibling text nodes that have no intervening non-text inline child and identical canonical mark-sets, concatenating their `value`s. Only a text node whose sole keys are `type`, `value`, and `marks` (after the step 1 stripping) is merge-eligible; a text node that also carries an `id`, `attributes`, or any other field is preserved unchanged and acts as a boundary, so no identifier or annotation is dropped. Offsets are defined over the concatenated text content (Anchors and References, section 3), so this moves no anchor. This rule and section 4.3.2 item 2 apply only within a **block-content array** — an array reached through one of the field names `blocks`, `caption`, `children`, `content`, `hints` or `preamble`, which is the exhaustive set. Membership is keyed on that field NAME and not on the owner's type: several such arrays hang off sub-objects carrying no `type` at all — a `subfigure`, an `academic:exercise` part, a `solution`, a structured abstract's section — and the content root, which is not a block, owns one. An implementation keying on the owner rather than the field misses them and computes a different document ID. Membership is keyed on the enclosing field, not on element shape: a text-shaped object an extension block carries in any other array is not block content and is left as authored, and `attributes` is a barrier at every depth beneath it — a closed set of authored values (Content Blocks, section 3.1) whose members are data even when shaped like inline content. An extension block therefore receives these transforms on a rich-text array exactly when it names that array with one of these keys; an array named anything else carries text the transforms do not reach, and a document containing one is still conformant. The two rules do not reach identical positions: this rule does not recurse into a text node's `marks`, so it never reaches a `presentation:footnote` mark's `content`, while section 4.3.2 item 2 does.
5. **Canonicalize identifiers**: relabel author-chosen identifiers to position-based canonical names, so that two documents differing only in their id *labels* produce the same document ID. A text node's `id` is outside this namespace entirely: it is never relabeled, occupies no index, and is not compared for duplicates — item 4 preserves it unchanged because it is content rather than a label. Because it survives into the canonical content, it MUST NOT spell a canonical name (`b` followed by digits); one that does is a canonicalization error, for the same reason the prohibition below applies to a preserved id — left as authored beside a generated `b0`, two identifiers reach the canonical output under one name. The relabeled namespace is the shared identifier namespace (Anchors and References, section 4): the `id` of every block other than a text node (including `namespace:type` extension blocks), the `id` of every `anchor` mark, and the `id` of every in-content sub-block element that carries one — an academic equation line and a `subfigure` — even though those carry no block `type`. These sub-block elements are the array items of an equation group's `lines` and a figure's `subfigures` — the exhaustive set of relabeled sub-block id arrays. Membership is keyed on WHERE an id sits (the enclosing field), not merely on whether the node has a `type` or is an array item: an id-bearing object an extension block carries in its own data array (any array other than `lines`/`subfigures`) is left as authored, even when shaped like a namespace member. In particular a `semantic:bibliography` entry's `id` — a CSL citation key in an `entries` array, referenced by a `citation` mark's `refs` (below) — is not relabeled, though the entry carries a CSL `type` that makes it resemble a block. Likewise an identifier carried on a singular named sub-object — such as a signature block's `signer` `id`, a Person identifier rather than a content-anchor target — is not part of this namespace. An id that would otherwise be a member of this namespace is left as authored when carried under `attributes`, keyed on POSITION: item 4 makes that a barrier at every depth, and Content Blocks section 3.1 closes it to authored values, so an id there is content — a vocabulary key a JSON-LD or CSL consumer resolves against — and not a label this rule may rename. Relabeling it applies alpha-equivalence to data the author did not choose as a label: two documents whose bibliographic citation keys differ entirely would otherwise share a document ID, so a signature over one verifies the other. This is the same reasoning that exempts a `semantic:bibliography` entry's `id` above, applied to the same data one field over. Two extension block types are a further exception, keyed on TYPE rather than position: `semantic:term` and `semantic:footnote`. Each is the **defining** side of a reference that addresses a separate namespace and is itself never relabeled — a `glossary` mark's `ref` and a `semantic:term` `see` resolve against a `semantic:term` block's `id` when no external glossary is declared, and a `footnote` mark resolves against a `semantic:footnote` block (Semantic Extension sections 4.5.2 and 8.3) — so relabeling the definition while leaving the reference as authored left those rules unsatisfiable on the canonical form: the block became `b0` while the reference still named `term-algorithm`. A preserved id — by position or by type — remains a full member of the namespace in every other respect: it occupies the shared uniqueness space, a duplicate is a canonicalization error exactly as above, and a Content Anchor URI resolves against it exactly as against a relabeled block id. Only the renaming skips it. Because it is left as authored, a TYPE-preserved id MUST NOT itself spell a canonical name (`b` followed by digits); one that does is a canonicalization error. This does not extend to an id preserved by POSITION: such an id is third-party vocabulary the author did not choose — `b1`, `b2` is a common reference-numbering convention — and rejecting it would fail exactly the documents the position rule exists to protect, while the same key in an extension block's own data array carries no such prohibition. This mirrors the prohibition on unresolved references below and holds for the same reason: duplicate detection compares *authored* ids, so an authored `b0` does not collide with the `b0` generated for some other block, and both would reach the canonical output under one identifier. Membership in the TYPE-keyed exception is keyed on the block's `type` rather than on where the id sits — unlike the rest of this item, and of necessity, since such an id sits directly on its block, exactly where any other block id sits. The `attributes` exception above is keyed on position, as the rest of this item is. Walk the canonical content depth-first — at each node assigning a name to the node's own `id` (if any) before recursing into its members (object members in JCS key order, array elements in index order) — and give each id-defining occurrence the next name in the sequence `b0`, `b1`, `b2`, …; a duplicate id within this namespace is a canonicalization error. This walk enters a text node's `marks`, and a mark's `content` with them — unlike item 4, which does not. An occurrence whose id is preserved takes no name and **consumes no index**: the sequence continues at the same name for the next relabeled occurrence, so a `semantic:term` followed by a paragraph yields `b0` on the paragraph. Reserving an index for the preserved occurrence instead is equally implementable and would give identical content a different document ID, so the choice is stated here rather than left to the implementation. Then rewrite every Content Anchor URI reference (`#id`, optionally followed by `/offset` or `/start-end`) whose id is one of these names — preserving the suffix — in the following reference fields: a `link` mark `href`; the `academic:theorem-ref`, `academic:equation-ref`, and `academic:algorithm-ref` mark `target`; the `academic:theorem` `uses`; the `academic:proof` `of`; and the `semantic:ref` and `presentation:reference` block `target`. A Content Anchor URI whose id is not a relabeled id — for example a cross-document anchor — is left unchanged, except that such an unresolved reference MUST NOT itself spell a canonical name (`b` followed by digits): an unresolved `#b0` is a canonicalization error, because left unchanged it would be indistinguishable from a reference that resolved to that block, collapsing two distinct documents onto one ID. After rewriting, the marks of **every** text node are sorted and de-duplicated by item 3's ordering rule, including a text node inside a mark's `content`.

   The following are left as authored, so a document that differs only in one of these labels is not yet guaranteed to share a document ID: the reference fields that address other namespaces — the `footnote` mark `id`, the `glossary` mark `ref`, `semantic:term` `see`, `citation` `refs`, `entity` `uri`, `index` `term`, and the `academic:algorithm-ref` `line` (a label into an algorithm's line numbering, not an `id`). Equation-line ids and subfigure ids are not in this set: they share the relabeled uniqueness namespace described above. A `semantic:footnote` or `semantic:term` block's own `id` is left as authored as well, but for a different reason and with a different consequence: it does not address another namespace — it stays in the shared uniqueness-and-anchor namespace described above and merely receives no canonical name, so anchors still resolve to it and a duplicate is still an error. It is listed here only because a document differing in one of those labels does not share a document ID. (A single text node is expected to carry at most one `anchor` mark; canonical names for multiple anchors on one node depend on their canonical mark order.)
...
1. **JSON Canonicalization**: Serialize using [RFC 8785](https://www.rfc-editor.org/rfc/rfc8785) (JCS). In particular:
   ...
   - Numbers are serialized per RFC 8785, which mandates the ECMAScript `Number.prototype.toString` algorithm over IEEE-754 double-precision values. Negative zero (`-0`) MUST be serialized as `0`. All hashed numbers are treated as IEEE-754 doubles, so producers MUST NOT rely on numeric precision beyond what a double round-trips; integers in hashed content MUST have magnitude at most 2^53 - 1 (the safe-integer limit). This is also a **consumer** obligation: a verifier MUST reject a document whose hashed content carries a non-finite number, or an integer whose magnitude exceeds 2^53 - 1, as a canonicalization error in every document state (State Machine section 5.4) — a value the double cannot represent exactly is not the value the signer committed to, so it MUST NOT be silently coerced and hashed.
   - Strings are escaped per RFC 8785

2. **Unicode Normalization**: Producers MUST emit all object keys and string values in Normalization Form C (NFC) and as well-formed Unicode (no unpaired surrogate code points). This applies to the concatenated text content of each maximal run of adjacent text nodes within a block-content array (item 4 above defines that set of arrays; a run here is bare adjacency and does NOT require the identical mark-sets item 4's merge does, so a run the merge leaves intact is still checked as a whole; for the concatenation procedure see Anchors and References, section 3), not merely to individual text-node values: an NFC-combining sequence MUST NOT be split across text-node boundaries. NFC is a property of the stored bytes, not a hash-time transform — implementations MUST NOT normalize while computing a hash, so the stored and hashed bytes are always identical. Content that is not already NFC is non-conformant. This is also a **consumer** obligation: a verifier MUST reject a document carrying an ill-formed or non-NFC hashed key or string value as a canonicalization error in every document state (State Machine section 5.4). It MUST NOT normalize the string and proceed — that would hash bytes the producer never stored, mint a second identity for the document, and defeat the rule above that the stored and hashed bytes are always identical.
